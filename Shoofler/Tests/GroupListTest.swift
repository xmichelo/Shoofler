...
struct GroupListTests {
    ...
    func testGroupsListFromArray() {
        ...
        #expect(GroupList(from: []).count == 0)
        
        let uuid = UUID()
        let groups = [
            Group(id: uuid, name: "Group 1"),
            Group(id: UUID(), name: "Group 2"),
            Group(id: uuid, name: "Group 3"),
        ]
        let groupList = GroupList(from: groups)
        #expect(groupList.count == 2)
